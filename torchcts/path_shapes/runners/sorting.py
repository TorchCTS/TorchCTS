...
from torchcts.path_shapes.runners.common import compare_output, float_data, run_device_op, torch_dtype
...
def run_sort(case: dict, device: str, compare) -> None:
    ...
    compare_output("sort", (actual_values, actual_indices), (expected_values, expected_indices), compare, category="exact", dtype=dtype, device=device, input=x_cpu, kwargs=kwargs)
...
def run_topk(case: dict, device: str, compare) -> None:
    ...
    compare_output("topk", (actual_values, actual_indices), (expected_values, expected_indices), compare, category="exact", dtype=dtype, device=device, input=x_cpu, args=(s["k"], s["dim"]), kwargs=kwargs)
...
def run_kthvalue(case: dict, device: str, compare) -> None:
    ...
    compare_output("kthvalue", (actual_values, actual_indices), (expected_values, expected_indices), compare, category="exact", dtype=dtype, device=device, input=x_cpu, args=(s["k"], s["dim"]), kwargs={"dim": s["dim"]})
